...
kill @e[type=item]
execute at @e[tag=base] if score -Special:On options matches -1 run setblock ~ ~-94 ~ air
execute at @e[tag=base] if score -Special:On options matches -1 run setblock ~ ~-94 ~ minecraft:vault[ominous=true]{config:{key_item:{count:1,id:"minecraft:ominous_trial_key"},loot_table:"skywars:chests/special_items/magikarp"}}